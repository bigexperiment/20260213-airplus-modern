...
import { Mountain, Map, Camera, Phone, Mail, ShieldCheck, Clock3, Leaf, Star } from "lucide-react";
...
export default async function Home() {
  ...
  const tripSteps = [
    { title: "Share your plan", detail: "Dates, fitness level, and dream route." },
    { title: "Get your custom itinerary", detail: "A practical route with acclimatization built in." },
    { title: "Arrive and trek confidently", detail: "Ground support, logistics, and guide briefing included." },
  ];

  const seasonCards = [
    { season: "Spring", months: "Mar - May", mood: "Rhododendrons, stable weather, ideal for first-time trekkers." },
    { season: "Autumn", months: "Sep - Nov", mood: "Crisp mountain views and peak trekking conditions." },
    { season: "Winter", months: "Dec - Feb", mood: "Quiet trails and dramatic landscapes for prepared hikers." },
  ];
  ...
  return (
    <div className="min-h-screen bg-noise aurora-bg">
      ...
      <section className="container-px pb-4">
        <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 md:px-6 md:py-4">
          <div className="grid gap-3 text-sm md:grid-cols-3">
            <div className="inline-flex items-center gap-2"><ShieldCheck className="size-4 text-primary" /> Licensed operators and verified guides</div>
            <div className="inline-flex items-center gap-2"><Clock3 className="size-4 text-primary" /> Fast trip planning and 24/7 local support</div>
            <div className="inline-flex items-center gap-2"><Leaf className="size-4 text-primary" /> Responsible trekking with low-impact routes</div>
          </div>
        </div>
      </section>
      ...
      {/* Journey flow */}
      <section className="section container-px">
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-primary/10 via-transparent to-accent/10 p-6 md:p-8">
          <h2 className="text-2xl md:text-3xl font-semibold">How Your Journey Works</h2>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">No guesswork. We handle route design, logistics, and on-ground coordination so you can focus on the trail.</p>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {tripSteps.map((step, idx) => (
              <div key={step.title} className="glass rounded-2xl p-5">
                <div className="text-xs font-semibold tracking-wide text-primary">STEP {idx + 1}</div>
      ...
      <section className="section container-px">
        <div className="mb-4 flex items-center gap-2 text-amber-300">
          ...
          <Star className="size-4 fill-current" />
          <Star className="size-4 fill-current" />
          <span className="ml-1 text-sm text-muted-foreground">Loved by trekkers from around the world</span>
        </div>
      ...
      <section className="section container-px">
        <div className="flex items-end justify-between mb-4">
          <h2 className="text-2xl md:text-3xl font-semibold">Best Time To Trek</h2>
          <Link href="#contact" className="text-sm text-muted-foreground hover:text-foreground">Get month-by-month advice</Link>
        </div>
      ...
      <section className="section container-px">
        <div className="rounded-3xl border border-white/10 p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gradient-to-r from-primary/10 to-accent/10">
          <div>
            ...
            <div className="text-sm text-muted-foreground">Tell us your dates and goals and get a free custom itinerary with realistic pace and logistics.</div>
          </div>
